...
- **Next step:** M9 — end-to-end scenario: seed a real multi-step plan (Aurel plans → agents travel/execute via OpenCode → REVIEW→COMPLETED), observability panel (event feed + task/agent timelines), approval stub (`[Approve]` gate before real work), minimap polish.
