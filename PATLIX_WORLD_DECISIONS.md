...
- [x] **M10 — Hardening:** agent state machine per role (pick only idle agents, reuse role-matched busy agents), prompt/workspace guardrails, run metrics + outbox cleanup, minimap zone labels.
...
- **Done:** **M10 complete** — hardening: **api** (`329dd89`) agent state machine per role (only idle agents get assigned; busy role-matched agents reused), prompt/workspace guardrails in the executor (agents constrained to the current repo, no commits), run metrics collection + outbox cleanup, self-healing bootstrap resume of ACTIVE plans, e2e health spec (`/api` prefix). **web** (`f486109` + `02bfaaf`) minimap zone labels for navigation, then integration fixes: port migration to `4204`/`3004` (was colliding with aurel-dashboard on `4203`/`3003`), Rapier `0.14`→`0.20` heightfield migration (vertex-based `(nrows+1)×(ncols+1)` column-major heights, scale = total plane size, centered on origin — no manual translation), and a camera-relative forward fix (W was walking toward the camera). Verified: web 22 vitest + api jest + e2e pass, `ng build` clean, tsc clean on both. patlix-world-web pushed (`02bfaaf`).
